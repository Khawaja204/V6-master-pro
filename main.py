...
# ── V6 DB Bridge: use SQLite when available, fallback to JSON ──
def _db_save(table: str, data):
    if _V6_UPGRADE:
        import v6_database as v6db
        {"paper_trades": v6db.save_paper_trades,
         "whale_copy_trades": v6db.save_whale_copy_trades,
         "backtest_signals": v6db.save_backtest_signals,
         "api_keys": v6db.save_api_keys,
         "clients": v6db.save_clients,
         "holdings": v6db.save_holdings,
         "learning_data": v6db.save_learning_data,
         "price_alerts": v6db.save_price_alerts}.get(table, lambda x: None)(data)

def _db_load(table: str, default=None):
    if _V6_UPGRADE:
        import v6_database as v6db
        return {"paper_trades": v6db.load_paper_trades,
                "whale_copy_trades": v6db.load_whale_copy_trades,
                "backtest_signals": v6db.load_backtest_signals,
...
if _ADMIN_PASSWORD_ENV:
    ...
    # SECURITY: no hardcoded fallback. Generate a random one-time password
    # and print it to the server log so the admin can retrieve it — far
    # safer than a public, guessable default like "786".
    ADMIN_PASSWORD = _secrets.token_urlsafe(12)
    print(f"\n{'='*70}\n[SECURITY] ADMIN_PASSWORD not set in environment.\n"
          f"Generated one-time password: {ADMIN_PASSWORD}\n"
          f"Set ADMIN_PASSWORD as a secret to persist this across restarts.\n{'='*70}\n")
...
if _V6_UPGRADE:
    try:
        _ld_loaded = _db_load("learning_data", default={})
    except Exception as _e:
        log.warning(f"[V6 DB] learning_data load failed: {_e}")
if _ld_loaded:
    _DEFAULT_LD.update(_ld_loaded)
else:
    try:
        ...
        log.debug(f"[INIT] learning_data.json not found or invalid — using defaults ({e})")
...
if _V6_UPGRADE:
    try:
        _enc_keys = _db_load("api_keys", default={})
        for _ex, _k in (_enc_keys or {}).items():
            _API_KEYS[_ex] = {
...
def _save_api_keys():
    ...
    if _V6_UPGRADE:
        try:
            _enc = {}
            for _ex, _k in _API_KEYS.items():
                _enc[_ex] = {
                    "api_key":    encrypt(_k.get("api_key", "")),
                    "secret_key": encrypt(_k.get("secret_key", "")),
                }
                if _k.get("passphrase"):
...
if _V6_UPGRADE:
    try:
        PAPER_TRADES = _db_load("paper_trades", default=[])
    except Exception as _e:
        log.warning(f"[V6 DB] paper_trades load failed: {_e}")
if not PAPER_TRADES:
    try:
        with open(_TRADES_FILE) as _ptf:
            PAPER_TRADES = json.load(_ptf)
    except Exception as e:
        log.debug(f"[INIT] paper_trades.json not found or invalid — starting empty ({e})")
_WC_TRADES_FILE = "whale_copy_trades.json"
WHALE_COPY_TRADES: list = []
if _V6_UPGRADE:
    try:
        WHALE_COPY_TRADES = _db_load("whale_copy_trades", default=[])
    except Exception as _e:
        log.warning(f"[V6 DB] whale_copy_trades load failed: {_e}")
if not WHALE_COPY_TRADES:
    try:
        with open(_WC_TRADES_FILE) as _wcf:
            WHALE_COPY_TRADES = json.load(_wcf)
    except Exception as e:
        log.debug(f"[INIT] whale_copy_trades.json not found or invalid — starting empty ({e})")
...
try:
    ...
    log.debug(f"[INIT] combo_trades.json not found or invalid — starting empty ({e})")
...
if _V6_UPGRADE:
    try:
        _loaded_bt = _db_load("backtest_signals", default=[])
    except Exception as _e:
        log.warning(f"[V6 DB] backtest_signals load failed: {_e}")
if not _loaded_bt:
    try:
        with open(_BACKTEST_FILE) as _bsf:
            _loaded_bt = json.load(_bsf)
    except Exception as e:
        log.debug(f"[INIT] backtest_signals.json not found or invalid — starting empty ({e})")
        _loaded_bt = []
try:
    ...
    log.warning(f"[INIT] Failed to filter backtest signals on load — starting empty ({e})")
...
def _save_backtest_signals():
    ...
    if _V6_UPGRADE:
        try:
            _db_save("backtest_signals", BACKTEST_SIGNALS[:100])
        except Exception as e:
            log.debug(f"[V6 DB] backtest_signals save failed: {e}")
_wc_dedup: dict = {}
_previous_walls    = {}
_alert_cooldown    = {}
_login_attempts    = {}
...
_ledger_lock = threading.Lock()   # guards PAPER_TRADES/WHALE_COPY_TRADES/COMBO_TRADES/BACKTEST_SIGNALS dedup+insert
...
# ── Weekly rolling drawdown (independent of the daily breaker) ──
_WEEKLY_TRIP_FILE = "weekly_circuit_state.json"
...
_weekly_stats_by_bot = _load_weekly_stats()
...
try:
    ...
    log.debug(f"[INIT] whale_copy_learning.json not found or invalid — using defaults ({e})")
            ...
            for tr in WHALE_COPY_TRADES:
                ...
                if hit_target or hit_sl or age >= _wc_timeout_hrs * 3600 or stale_exit2:
                    ...
                    if tr.get("direction") == "COPY_BUY":
                        ...
                        _record_weekly_pnl("wall", tr.get("pnl_pct") or 0.0)
...
# Bypassed:     send_telegram(telegram_msg)
...
def notify_trade(symbol: str, side: str, strategy: str, mode: str, reason: str,
    ...
    msg = "\n".join(lines)
# Bypassed:     send_telegram(msg, inline_button={"text": f"📊 Sniper: {symbol.replace('USDT','')}",
        ...
        if entry:   # an actual entry passed the GREEN-only + circuit-breaker gate
            ...
            if not GLOBAL_DATA.get("paper_mode", True):
                ...
                if _real_res.get("ok"):
                    ...
                    if _V6_OCO and "BINANCE" in _API_KEYS and coin.get("price"):
                        try:
                            _fill_qty = round(_real_amt / coin["price"], 6)
                            _oco_res = GLOBAL_DATA["oco_manager"].place_oco(
                                symbol=coin["symbol"], side="SELL", quantity=_fill_qty,
                                price=tp_zones.get("tp1", 0),
                                stop_price=tp_zones.get("stop_loss", 0),
                                stop_limit_price=tp_zones.get("stop_loss", 0),
                                api_key=_API_KEYS["BINANCE"]["api_key"],
                                secret_key=_API_KEYS["BINANCE"]["secret_key"],
                            )
                            if _oco_res.get("ok"):
                                reason += " | 🎯 OCO bracket placed (TP1+SL)"
                            audit("SYSTEM", "AUTO_OCO_BRACKET",
                                  "OK" if _oco_res.get("ok") else "FAILED",
                                  f"sym={coin['symbol']} qty={_fill_qty}")
                        except Exception as _oe:
                            log.warning(f"[V6 OCO] bracket placement failed: {_oe}")
                else:
            ...
            if _V6_PTP:
                try:
                    GLOBAL_DATA["ptp_manager"].add_position(
                        symbol=coin["symbol"], side="BUY",
                        entry_price=coin["price"],
                        qty=(GLOBAL_DATA.get("fund_limit_usdt", 10.0) / coin["price"]) if coin.get("price") else 0,
                        tp1=tp_zones.get("tp1", 0), tp2=tp_zones.get("tp2", 0),
                        tp3=tp_zones.get("tp3", 0), sl=tp_zones.get("stop_loss", 0),
                        mode="PAPER" if GLOBAL_DATA.get("paper_mode", True) else "REAL",
                        order_id=entry.get("id", ""),
                    )
                except Exception as _pe:
                    log.debug(f"[V6 PTP] add_position failed: {_pe}")
...
_WHALE_HISTORY_FILE = "whale_history.jsonl"
_WHALE_HISTORY_MAX_LINES = 500000   # ~months of data at typical scan volume before rotation

def _log_whale_history(inst_signals: list, whale_data: list, cycle: int) -> None:
    """Appends one compact JSON line PER SCANNED COIN, every cycle — building
    a real historical record of whale_power/OBI/traffic-light state that
    Binance itself does not retain. Once a few months of this accumulate,
    it can be joined against price history to backtest the Whale/Institutional
    and Smart-Money categories that a pure price/volume backtest cannot
    reconstruct. Rotates the file once it exceeds _WHALE_HISTORY_MAX_LINES
    so it never grows unbounded on the free-tier disk."""
    try:
        whale_by_sym = {w["symbol"]: w for w in whale_data}
        ts = time.time()
        lines = []
        for s in inst_signals:
            ...
            if not sym:
                continue
            inst = s.get("inst", {}) or {}
            wh = whale_by_sym.get(sym, {})
            row = {
                "ts": round(ts), "cycle": cycle, "sym": sym,
                "price": s.get("price", 0),
                "wp": inst.get("whale_power", 0),
                "obi": (wh.get("obi", {}) or {}).get("obi", 0),
                "obi_vel": (wh.get("obi", {}) or {}).get("velocity", 0),
                "traffic": inst.get("traffic", "RED"),
                "inst_score": inst.get("inst_score", 0),
                "confirms": inst.get("confirms", 0),
                "spike": bool(inst.get("spike", False)),
                "spoof": bool((wh.get("spoofing", {}) or {}).get("bid_spoof") or
                             (wh.get("spoofing", {}) or {}).get("ask_spoof")),
                "b2p": bool(wh.get("blink_to_push", False)),
            }
            lines.append(json.dumps(row))
        if not lines:
            return
        with open(_WHALE_HISTORY_FILE, "a") as f:
            f.write("\n".join(lines) + "\n")

        # Rotation check — cheap line-count guard so this never eats the disk
        if cycle % 200 == 0:
            try:
                with open(_WHALE_HISTORY_FILE) as f:
                    count = sum(1 for _ in f)
                if count > _WHALE_HISTORY_MAX_LINES:
                    with open(_WHALE_HISTORY_FILE) as f:
                        all_lines = f.readlines()
                    keep = all_lines[-(_WHALE_HISTORY_MAX_LINES // 2):]
                    with open(_WHALE_HISTORY_FILE, "w") as f:
                        f.writelines(keep)
                    log.info(f"[WHALE-HISTORY] Rotated — kept last {len(keep)} lines")
            except Exception as _re:
                log.debug(f"whale history rotation check failed: {_re}")
    except Exception as e:
        log.debug(f"whale history log failed: {e}")
...
def _compute_live_signal(symbol: str) -> dict:
    ...
    if _V6_MTF:
        try:
            signal["mtf"] = get_mtf_signal(symbol, fetch_klines)
        except Exception as _me:
            log.debug(f"[V6 MTF] {symbol} failed: {_me}")
    return signal
...
def _save_learning_data():
    ...
    if _V6_UPGRADE:
        try:
            _db_save("learning_data", ld)
        except Exception as e:
            log.debug(f"[V6 DB] learning_data save failed: {e}")
...
def data_refresh_loop():
    ...
    while True:
        ...
        try:
            ...
            # ── Whale History Logger: builds our own historical dataset for
            # future whale/OBI backtesting (Binance retains none of this) ──
            _log_whale_history(inst_signals, whale_data, cycle)
...
def heartbeat_loop():
    ...
    while True:
        ...
        try:
            ...
            log.warning(f"Heartbeat loop error (thread stays alive): {e}")
...
@app.before_request
def _csrf_protect():
    """Rejects any /admin/* POST that doesn't carry a valid, session-matching
    CSRF token — blocks a malicious page from silently triggering admin
    actions (kill switch, real-mode toggle, fund changes, etc.) while an
    admin session happens to be active in the browser."""
    if request.method == "POST" and request.path.startswith("/admin/") and request.path != "/admin/login":
        token = request.form.get("csrf_token", "")
        session_token = session.get("csrf_token", "")
        if not session_token or not hmac.compare_digest(token, session_token):
            audit(request.remote_addr, "CSRF_BLOCKED", "REJECTED", f"path={request.path}")
            return "<script>alert('Session expired or invalid request — please reload the admin page and try again.');window.location=\'/admin\'</script>", 403
...
<form method="POST"><input type="hidden" name="csrf_token" value="{{ csrf_token }}"/><input type="password" name="password" placeholder="Master Password" autofocus/>
...
_last_external_ping = {"ts": None}
_UPTIME_LOG_FILE = "uptime_log.jsonl"

def _log_uptime_event(event_type: str, note: str = "", source_ip: str = ""):
    """Appends one line to uptime_log.jsonl — rolling record of every
    process STARTUP and every external PING (e.g. UptimeRobot hitting
    /health). Capped at the last 2000 entries so the file never grows
    unbounded. Used to diagnose Render spin-downs/restarts after the fact."""
    try:
        entry = {"type": event_type, "ts": time.time(),
                 "pkt": _pkt_ts(), "note": note,
                 "pid": os.getpid(), "source_ip": source_ip}
        lines = []
        try:
        ...
        lines = lines[-2000:]
        with open(_UPTIME_LOG_FILE, "w") as f:
            f.writelines(lines)
    except Exception as e:
        log.debug(f"uptime log write failed: {e}")
...
def _load_clients() -> list:
    if _V6_UPGRADE:
        try:
            rows = _db_load("clients", default=[])
            if rows:
                return rows
        except Exception as e:
            log.debug(f"[V6 DB] clients load failed: {e}")
    try:
...
def _save_clients(clients: list):
    ...
    if _V6_UPGRADE:
        try:
            _db_save("clients", clients)
        except Exception as e:
            log.debug(f"[V6 DB] clients save failed: {e}")
...
def _load_holdings() -> list:
    if _V6_UPGRADE:
        try:
            rows = _db_load("holdings", default=[])
            if rows:
                return rows
        except Exception as e:
            log.debug(f"[V6 DB] holdings load failed: {e}")
    try:
...
def _save_holdings(holdings: list):
    ...
    if _V6_UPGRADE:
        try:
            _db_save("holdings", holdings)
        except Exception as e:
            log.debug(f"[V6 DB] holdings save failed: {e}")
...
@app.route("/admin/weekly_breaker_status")
@_admin_required
...
@app.route("/admin/reset_weekly_breaker", methods=["POST"])
@_admin_required
...
@app.route("/admin/whale_history_status")
@_admin_required
...
@app.route("/admin/whale_history_csv")
@_admin_required
...
@app.route("/admin/uptime_monitor_status")
@_admin_required
...
@app.route("/admin/live_binance_holdings")
@_admin_required
...
<form method="POST"><input type="hidden" name="csrf_token" value="{{ csrf_token }}"/><input type="text" name="username" placeholder="Username"/>
...
def _verify_client_local(username: str, password: str):
    """Verify client against local clients.json file. Supports both hashed
    (werkzeug pbkdf2/scrypt) and legacy plaintext passwords — a legacy
    plaintext match is silently upgraded to a hash on successful login."""
    try:
        with open("clients.json") as _f:
            clients = json.load(_f)
        for c in clients:
            if c.get("name", "").strip() != username:
                continue
            stored = c.get("password", "")
            is_hashed = stored.startswith(("pbkdf2:", "scrypt:"))
            matched = check_password_hash(stored, password) if is_hashed else (stored.strip() == password)
            if matched:
                if not is_hashed:
                    # Legacy plaintext entry — upgrade to a hash now that we've verified it
                    c["password"] = generate_password_hash(password)
                    try:
                        with open("clients.json", "w") as _wf:
                            json.dump(clients, _wf, indent=2)
                    except Exception as _me:
                        log.debug(f"Password hash migration write failed: {_me}")
...
#     )
    ...
    if _V6_PTP:
        ...
        GLOBAL_DATA["ptp_manager"].start_monitor(fetch_ticker_price)
