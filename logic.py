...
def _macd_hist_series(closes: list, fast: int = 12, slow: int = 26, signal: int = 9) -> list:
    """Elementwise MACD histogram across the whole series (for backtesting,
    where we need a value at every bar, not just the latest)."""
    if len(closes) < slow:
...
def _volume_ratio_series(volumes: list, window: int = 20) -> list:
    """volumes[i] / rolling-median(volumes[i-window:i]) — backtest proxy
    for the live volume-surge detector (which compares against the median
    of the current scan's coin universe)."""
    n = len(volumes)
    out = [1.0] * n
    for i in range(window, n):
        win = sorted(volumes[i - window:i])
        median = win[len(win) // 2] if win else 0
        out[i] = round(volumes[i] / median, 2) if median else 1.0
    return out


def _backtest_v6_score(rsi_val: float, macd_hist_val: float, price: float,
                       vol_ratio: float, btc_volatility_pct: float,
                       coin_chg_pct: float, regime: str) -> dict:
    """
    Backtest-compatible SUBSET of the live compute_v6_final_score(): only the
    Market Regime (10), Technical (12), and Trade Engine/Risk (10) categories
    — 32 of the live engine's 54 points. The Whale/Institutional (12) and
    Smart-Money Divergence (10) categories are DELIBERATELY EXCLUDED here:
    they depend on live order-book snapshots and on-chain flow, which Binance
    does not retain historically, so no backtest (by us or anyone else) can
    reconstruct them for a past timestamp. Traffic-light tier below is a
    technical-strength proxy standing in for the missing whale confirmation
    — it is NOT equivalent to a live whale-confirmed GREEN light.
    Score is rescaled to /100 over the 32 available points, using the same
    68/45 BUY/WAIT/AVOID thresholds as the live engine for consistency.
    """
    # ── Market Regime (10) ──
    reg_trend = {"TRENDING": 3, "RANGING": 2, "VOLATILE": 1}.get(regime, 2)
    reg_trend += 2 if coin_chg_pct > 0.5 else 1 if coin_chg_pct >= -0.5 else 0
    reg_trend  = min(5, reg_trend)
    v = abs(btc_volatility_pct or 0)
    reg_vol = 5 if v < 2 else 3 if v < 4 else 2 if v < 6 else 1
    market_regime = reg_trend + reg_vol
    ...
    # ── Technical (12) ──
    rsi_v = rsi_val or 50
    if 40 <= rsi_v <= 55:             rsi_pts = 4
    elif 55 < rsi_v <= 62 or 35 <= rsi_v < 40: rsi_pts = 3
    elif 30 <= rsi_v < 35:            rsi_pts = 2
    elif rsi_v > 70 or rsi_v < 25:    rsi_pts = 0
    else:                             rsi_pts = 1
    macd_rel = (macd_hist_val / price * 100) if price else 0
    if macd_rel > 0.05:    macd_pts = 4
    elif macd_rel > 0:     macd_pts = 3
    elif macd_rel == 0:    macd_pts = 2
    elif macd_rel > -0.05: macd_pts = 1
    else:                  macd_pts = 0
    in_surge = vol_ratio >= 2.5
    vol_pts  = 4 if in_surge else 1
    technical = rsi_pts + macd_pts + vol_pts

    # ── Trade Engine & Risk (10) ──
    # RR is near-constant here because ATR TP/SL multipliers are fixed config
    # values — this mirrors the live formula faithfully, it is simply a
    # property of using fixed ATR multipliers rather than a backtest artifact.
    rr = 1.5  # (tp1 - entry_low) / (entry_low - sl) at default 1.5x/1.5x ATR multipliers
    rr_pts = 4  # corresponds to the 1.5x rr_pts bucket in the live formula
    tech_strength = rsi_pts + macd_pts + vol_pts
    proxy_traffic = "GREEN" if tech_strength >= 8 else "YELLOW" if tech_strength >= 5 else "RED"
    tr_pts = 5 if proxy_traffic == "GREEN" else 2 if proxy_traffic == "YELLOW" else 0
    trade_engine = rr_pts + tr_pts

    raw = market_regime + technical + trade_engine
    score = max(0, min(100, round(raw / 32 * 100)))
    if score >= 68:   label = "BUY"
    elif score >= 45: label = "WAIT"
    else:             label = "AVOID"

    return {"score": score, "label": label, "proxy_traffic": proxy_traffic,
            "breakdown": {"market_regime": market_regime, "technical": technical,
...
def historical_backtest(symbols: list, months: int = 3, interval: str = "1h",
    ...
    timeout_bars = 48
    v6_buy_threshold = 68  # matches the live engine's BUY threshold

    # ── BTC regime series (volatility + change, rolling 20-bar window) ────
    btc_vol_by_ts, btc_chg_by_ts, btc_regime_by_ts = {}, {}, {}
    btc_raw = fetch_klines_range("BTCUSDT", interval, start_ms, now_ms)
    if btc_raw:
        b_opens = [int(k[0]) for k in btc_raw]
        b_high  = [float(k[2]) for k in btc_raw]
        b_low   = [float(k[3]) for k in btc_raw]
        b_close = [float(k[4]) for k in btc_raw]
        for j in range(20, len(b_opens)):
            wh = max(b_high[j - 20:j + 1]); wl = min(b_low[j - 20:j + 1])
            vol_pct = (wh - wl) / wl * 100 if wl else 0
            chg_pct = (b_close[j] - b_close[j - 20]) / b_close[j - 20] * 100 if b_close[j - 20] else 0
            btc_vol_by_ts[b_opens[j]] = vol_pct
            btc_chg_by_ts[b_opens[j]] = chg_pct
            btc_regime_by_ts[b_opens[j]] = detect_market_regime(vol_pct, chg_pct)
    ...
    for sym in symbols:
        ...
        macd_h = _macd_hist_series(closes)
        vratio = _volume_ratio_series(vols)
        pos = None
        sym_trades = 0
        for i in range(30, len(closes)):
            if pos is None:
                if atr[i] <= 0:
                    continue
                coin_chg = (closes[i] - closes[i - 1]) / closes[i - 1] * 100 if closes[i - 1] else 0
                btc_vol  = btc_vol_by_ts.get(opens[i], 3.0)
                btc_chg  = btc_chg_by_ts.get(opens[i], 0.0)
                regime   = btc_regime_by_ts.get(opens[i], "RANGING")
                sc = _backtest_v6_score(rsi[i], macd_h[i], closes[i], vratio[i],
                                        btc_vol, coin_chg, regime)
                if sc["label"] == "BUY" and sc["score"] >= v6_buy_threshold:
                    e = closes[i]; a = atr[i]
                    pos = {
    ...
    return {
        ...
        "note": ("Entries now use a backtest-compatible subset of the live V6 54-point "
                "engine: Market Regime + Technical + Trade Engine/Risk (32 of 54 points, "
                "rescaled to /100, same 68/45 BUY/WAIT thresholds). The Whale/Institutional "
                "and Smart-Money Divergence categories (22 pts) are excluded — they need "
                "live order-book/on-chain data that Binance does not retain historically, "
                "so no backtest can reconstruct them for a past timestamp."),
    }
...
_whale_copy_state_lock = threading.Lock()
...
def detect_whale_copy_signals(whale_data: list, config: dict, market_regime: str = "RANGING") -> list:
    ...
    for w in whale_data:
        ...
        # persist_window enforcement: if the previous sighting is older than
        # persist_window_seconds, treat it as stale and restart the count —
        # otherwise a confirmation from hours ago could combine with a fresh
        # scan to falsely satisfy the confirmation requirement.
        persist_window = wc_cfg.get("persistence_window_seconds", 120)
        with _whale_copy_state_lock:
            prev = _whale_copy_state.get(sym)
            stale = bool(prev) and (now - prev.get("last_seen", 0) > persist_window)
            count = (prev["count"] + 1) if (prev and prev["direction"] == direction and not stale) else 1
...
def detect_combo_signals(inst_signals: list, whale_copy_signals: list, config: dict) -> list:
    """
    COMBO CONFLUENCE BOT — fires only when BOTH independent engines agree on
    the SAME coin at the SAME time:
      • V6 54-point score says v6.label == "BUY" (score >= combo.min_v6_score)
      • Whale Copy engine says direction == "COPY_BUY" and confirmed == True
        (confidence >= combo.min_wall_confidence)
    Exit is deliberately NOT computed here — combo trades are managed and
    closed exclusively by the V6 ATR/trailing-stop machinery downstream
    (config: bots.combo.exit_uses_v6_only), so this only returns entry
    candidates with the levels needed to open a position.
    """
    combo_cfg   = config.get("bots", {}).get("combo", {})
    min_v6      = combo_cfg.get("min_v6_score", 68)
    min_wall_c  = combo_cfg.get("min_wall_confidence", 50)

    # Index V6 buy signals by symbol — keep the HIGHEST-scoring signal per
    # symbol, not simply the first one encountered in the list.
    v6_buys = {}
    for s in inst_signals:
        v6 = s.get("v6", {}) or {}
        if v6.get("label") == "BUY" and v6.get("score", 0) >= min_v6:
            sym = s.get("symbol")
            if not sym:
                continue
            existing = v6_buys.get(sym)
            if not existing or v6.get("score", 0) > (existing.get("v6", {}) or {}).get("score", 0):
                v6_buys[sym] = s

    combos = []
    for wc in whale_copy_signals:
        if wc.get("direction") != "COPY_BUY":
            continue
        if not wc.get("confirmed"):
            continue
        if wc.get("confidence", 0) < min_wall_c:
            continue
        sym = wc.get("symbol")
        v6_match = v6_buys.get(sym)
        if not v6_match:
            continue

        tp = v6_match.get("tp_zones", {}) or {}
        v6 = v6_match.get("v6", {}) or {}
        combos.append({
            "symbol":          sym,
            "price":           v6_match.get("price", wc.get("price", 0)),
            "v6_score":        v6.get("score", 0),
            "v6_label":        v6.get("label", ""),
            "wall_confidence": wc.get("confidence", 0),
            "wall_price":      wc.get("wall_price", 0),
            "wall_size_usdt":  wc.get("wall_size_usdt", 0),
            "obi":             wc.get("obi", 0),
            # ── Entry/exit levels: sourced from V6's ATR engine only ──────
            "entry_low":       tp.get("entry_low", 0),
            "entry_high":      tp.get("entry_high", 0),
            "stop_loss":       tp.get("stop_loss", 0),
            "tp1":             tp.get("tp1", 0),
            "tp2":             tp.get("tp2", 0),
            "tp3":             tp.get("tp3", 0),
            "reason": (f"COMBO: V6-BUY {v6.get('score',0)} + Wall-COPY_BUY "
                       f"{wc.get('confidence',0)}% confluence on {sym}"),
            "detected_at": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time() + 5 * 3600)),
        })

    combos.sort(key=lambda x: (x["v6_score"] + x["wall_confidence"]), reverse=True)
    return combos
